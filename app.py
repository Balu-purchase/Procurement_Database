...
# --- 3. DASHBOARD PAGE ---
    ...
    # --- 🟢 NON-BOM TEAM MODULE ---
    if st.session_state.role == "NONBOMTEAM" or (menu == "NONBOM"):
                ...
                with st.form("adv_form", clear_on_submit=True):
                    ...
                    if st.form_submit_button("SUBMIT REQUEST"):
                        st.session_state.advance_payments.append({
                            "SUBMIT DATE": str(datetime.now().date()), "VENDOR NAME": v_name, 
                            "TYPE": v_type, "PI/INVOICE NO": v_pi, "INVOICE DATE": str(v_idate), 
                            "PO NO": v_po, "PO DATE": str(v_pdate), "AMOUNT": v_amt, "REMARKS": v_rem,
                            "PAYMENT STATUS": "PENDING", "MATERIAL STATUS": "PENDING", "GRN No": "", 
                            "PIC": "", "TAX INVOICE NO": "", "ACCOUNTING STATUS": "PENDING", 
                            "PAID ON": "", "RECEIVED ON": ""
                        })
                        st.rerun()

            st.subheader("Advance Payment Tracker (Editable for Non-BOM Only)")
            if st.session_state.advance_payments:
                df_adv = pd.DataFrame(st.session_state.advance_payments)
                if st.session_state.role == "NONBOMTEAM":
                    # Non-BOM Team Edits directly here
                ...
                with st.form("mis_form", clear_on_submit=True):
                    ...
                    if st.form_submit_button("SUBMIT MIS"):
                        st.session_state.mis_data.append({
                            ...
                            "RECEIVED QTY": 0, "PENDING QTY": m_qty, "GRN NO": "", "GRNDATE": "",
                            "PAYMENT STATUS": "INCOMPLETE", "PAYMENT DATE": "", "MATERIAL RECEIVE DATE": "", 
                            "INVOICE NO": "", "ACCOUNTING STATUS": "PENDING"
                        })
                        st.rerun()

            st.subheader("MIS Tracker (Editable for Non-BOM Only)")
    ...
    # --- 🔵 BOM & AUDIT ---
        ...
        st.info("BOM Module logic remains unchanged.")
    elif menu == "AUDIT LOGS":
        st.header("📝 Audit Logs")
        st.write("Complete record of system entries.")
