...
if not st.session_state.secure_access:
    ...
    with col2:
        ...
        # Broken into small pieces to avoid "unterminated string" errors
        st.markdown("<div style='background:rgba(0,0,0,0.8); padding:30px; border-radius:15px; border:2px solid #38bdf8; text-align:center;'>", unsafe_allow_html=True)
        st.markdown("<h1>🔐 SKYQUAD SHIELD</h1>", unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        st.markdown(f'<div class="status-box"><b>STATUS:</b><br>LIVE SYNC ACTIVE</div>', unsafe_allow_html=True)
        if st.button("LOGOUT"):
            st.session_state.secure_access = False
            st.rerun()
    ...
    if df is not None:
        ...
        m2.metric("Last Sync", datetime.now().strftime("%H:%M:%S"))
        ...
        st.error("📡 Link Error: Ensure Google Sheet is shared as 'Anyone with link can view'.")
